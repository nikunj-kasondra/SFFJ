...
class ListCircularController: UIViewController {
    ...
    func callLivreAPI() {
        if !Reachability.isConnectedToNetwork() {
            ...
            var urlStr = String()
            if flag == "S" {
                urlStr = "http://api.sainte-famille.edu.lb/api/login/getcirculaire?classId=\(UserDefaults.standard.value(forKey: "role") as! String)"
                
            } else if flag == "P" {
                urlStr = "http://api.sainte-famille.edu.lb/api/login/getcirculaire?classId=\(UserDefaults.standard.value(forKey: "role") as! String)"
            } else if flag == "T" {
                urlStr = "http://api.sainte-famille.edu.lb/api/login/getteahcercirculaire?userName=\(UserDefaults.standard.object(forKey: "Code") as! String)"
    ...
    func downLoadFile(indexPath: Int) {
        ...
        if fileManager.fileExists(atPath:writablePath) {
            ...
            var docUrl = String("http://api.sainte-famille.edu.lb/api/fileutils/GetSecureFileDownload?secureName=Circulaire&fileName=") + String(fileStr.replacingOccurrences(of: " ", with: "%20"))
...
extension ListCircularController: UITableViewDelegate,UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let htmlStr = (livreArr.value(forKey: "Subject") as! NSArray).object(at: indexPath.row) as? String
        let attrStr = try! NSAttributedString(
            data: (htmlStr?.data(using: String.Encoding.unicode, allowLossyConversion: true)!)!,
            options: [ NSDocumentTypeDocumentAttribute: NSHTMLTextDocumentType],
            documentAttributes: nil)
        
        cell.lblDesc.text = htmlStr
...
extension ListCircularController: UIDocumentInteractionControllerDelegate {
