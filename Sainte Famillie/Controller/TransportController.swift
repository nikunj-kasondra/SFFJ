...
class TransportController: UIViewController {
    ...
    @IBOutlet weak var regionView: UIWebView!
    ...
    override func viewDidLoad() {
        ...
        btnRegion.setTitleColor(UIColor.init(red: 255/255.0, green: 255/255.0, blue: 255/255.0, alpha: 0.5), for: .normal)
    ...
    @IBAction func btnRegionClicked(_ sender: Any) {
        ...
        inscView.isHidden = true
        btnInsc.setTitleColor(UIColor.init(red: 255/255.0, green: 255/255.0, blue: 255/255.0, alpha: 0.5), for: .normal)
        btnRegion.setTitleColor(UIColor.init(red: 255/255.0, green: 255/255.0, blue: 255/255.0, alpha: 1.0), for: .normal)
    ...
    func callPresentation() {
        if !Reachability.isConnectedToNetwork() {
            ...
            let url = URL(string: "http://api.sainte-famille.edu.lb/api/servicesauxiliaires/getramassagescolairepresentation")
    ...
    func callInsp() {
        if !Reachability.isConnectedToNetwork() {
            ...
            Progress.show(toView: self.view)
            let url = URL(string: "http://api.sainte-famille.edu.lb/api/servicesauxiliaires/getramassagescolaireinscriptions")
            URLSession.shared.dataTask(with: url!, completionHandler: {
                (data, response, error) in
                if(error != nil){
                    Progress.hide(toView: self.view)
    ...
    func callRegion() {
        if !Reachability.isConnectedToNetwork() {
            ...
            Progress.show(toView: self.view)
            let url = URL(string: "http://api.sainte-famille.edu.lb/api/servicesauxiliaires/getramassagescolaireregionsdesservies")
            URLSession.shared.dataTask(with: url!, completionHandler: {
                (data, response, error) in
                    ...
                    do{
                        ...
                        if self.regionArr.count != 0 {
                            ...
                            if self.regionHtmlStr != "" {
                                self.regionView.loadHTMLString(self.regionHtmlStr, baseURL: nil)
                            } else {
                                //self.lblPres.isHidden = false
                            }
...
extension TransportController:UIWebViewDelegate {
