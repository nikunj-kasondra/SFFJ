...
class AnnocesController: UIViewController {
    ...
    func callLivreAPI() {
        if !Reachability.isConnectedToNetwork() {
            ...
            let url = URL(string: "http://api.sainte-famille.edu.lb/api/login/getannonces?classId=\(classId)")
...
extension AnnocesController: UITableViewDelegate,UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        AnnocesDetailController.Id = ((self.livreArr.value(forKey: "Code") as! NSArray).object(at: indexPath.row) as? String)!
        let VC = Common.storyboard.instantiateViewController(withIdentifier: "AnnocesDetailController")
        Navigation.push_POP_to_ViewController(destinationVC: VC, navigationsController: Common.navigationController, isAnimated: true)
    }
}
extension AnnocesController: LBZSpinnerDelegate {
